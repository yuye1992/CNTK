...
public:
    ...
    void Validate(bool isFinalValidationPass) override
    {
        ...
        {
            bool inputHasFreeDimension = (m_convEng == nullptr) ? false : // For first minibatch, this flag must be false, so initial mem allocation can happen.
                                          (outputShape != m_convEng->Geometry()->OutputShape()) || (inputShape != m_convEng->Geometry()->InputShape());
            if ((m_convEng == nullptr) || ((m_convEng != nullptr) && inputHasFreeDimension))
            {
