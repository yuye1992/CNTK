...
void TestLearnerSerialization(int numParameters, const DeviceDescriptor& device)
{
    ...
    auto learner1 = SGDLearner(parameters, LearningRateSchedule(0.05));
    ...
    auto learner2 = SGDLearner(parameters, LearningRateSchedule( 0.05));
...
TrainerPtr BuildTrainer(const FunctionPtr& function, const Variable& labels,
                     LearningRateSchedule lr = LearningRateSchedule(0.005),
...
void TestLegacyModelSaving(const DeviceDescriptor& device)
{
    ...
    LearningRateSchedule learningRateSchedule2({ { 0.04, 0.02, 0.01, 0.008, 0.004, 0.002, 0.001 } }, actualMBSize);
...
BOOST_AUTO_TEST_CASE(LearnerSerializationBackcompat)
{
    ...
    auto learner = MomentumSGDLearner(net->Parameters(), LearningRateSchedule(0.005),
